...
async function fetchQuranData() {
    try {
        ...
        const [transRes, arabicRes, metaRes] = await Promise.all([
            fetch(`./shared-data/${AppState.translationFile}`), fetch('./shared-data/quran-simple.xml'), fetch('./shared-data/surah_meta.json')
        ]);

        if (!transRes.ok || !arabicRes.ok || !metaRes.ok) throw new Error("Files not found");
        ...
        AppState.surahMeta = await metaRes.json();
...
function renderLibraryList() {
    ...
    AppState.quranXML.querySelectorAll('sura').forEach(sura => {
        ...
        const meta = AppState.surahMeta[index - 1];
...
function displaySurah(index, targetVerse = null) {
    ...
    const meta = AppState.surahMeta[AppState.currentSurahIndex - 1];
...
function initFeedDelegation() {
    const handleAction = (e, isLongPress) => {
        ...
        const metaName = AppState.surahMeta[sIdx - 1].name;
...
function updateTabUI(tab) {
    ...
    if (tab === 'arabic') {
        ...
        titleEl.innerText = AppState.surahMeta[AppState.currentSurahIndex - 1]?.name || '';
